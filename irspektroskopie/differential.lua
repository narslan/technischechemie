...
a,b=fs.readcsv("data/1cm1scan.txt",0)
...
local f= io.open("data/diffpzweig1cm1Scan.txt","w")
for i=1,#erste_ableitung do
  --print(string.format("%.2f %.2f",erste_ableitung[i][1],erste_ableitung[i][2]))
  f:write(string.format("%.2f %.2f \n",erste_ableitung[i][1],erste_ableitung[i][2]))
end
f:close()

local f= io.open("data/diff2pzweig1cm1Scan.txt","w")
