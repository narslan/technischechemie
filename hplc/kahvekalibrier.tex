% !TEX encoding   = UTF-8
% !TEX program    = LuaLaTeX
% !TEX spellcheck = de_DE
\documentclass[a4paper]{article}
\usepackage{fontspec}
\usepackage[usenames,dvipsnames]{color}
\usepackage{siunitx}
\usepackage[german]{babel}

\usepackage{pgfplots}
\usepackage{pgfplotstable}
\pgfplotsset{compat=newest}
\usepgfplotslibrary{units}
\sisetup{detect-all}

\usepackage{luacode}
\definecolor{skyblue1}{rgb}{0.447,0.624,0.812}
\definecolor{scarletred1}{rgb}{0.937,0.161,0.161}
\definecolor{chameleon1}{rgb}{0.541,0.886,0.204}
%\usepackage{helvet}
\usepackage[landscape]{geometry}
\usepackage{unicode-math}

%\usepackage[landscape]{geometry}
%\setmainfont{Ubuntu}
\setmainfont{TeX Gyre Heros}

\setmathfont{TeX Gyre Heros}
\usepackage{fancyhdr}
\pagestyle{fancy}


\rhead{ Gruppe 24}
\lhead{ \textbf{HPLC - Coffein Bestimmung} die Chromatogramme der Kalibriermessungen}
\setlength{\headheight}{20pt}
\pagenumbering{gobble}
\usetikzlibrary{pgfplots.groupplots}

%\usepackage{libertine}
%\usepackage{beramono}
%\setmainfont{Ubuntu}
\begin{luacode*}

function drawA()
local string = string
local lines = {}

for line in io.lines("daten/kahve.txt") do
  table.insert(lines, line)
end
local summex=0
local summey=0
local xwerte={}
local ywerte={}



for i, line in ipairs(lines) do
  if i>1 then
     local s, e = line:find("%s+", 1)
     local y=tonumber(line:sub(e))
     local x=tonumber(line:sub(1,e))
     table.insert(xwerte,x)
     table.insert(ywerte,y)
   end
end

----m
local n = #lines-1

    tex.sprint("\\addplot[".."no marks".."] coordinates{")
    -- tex.sprint("\\addplot coordinates{")
    for i=1,n do
      tex.sprint("("..xwerte[i]..","..ywerte[i]..")")
    end
    tex.sprint("};")
    -- tex.sprint("\\node[small dot,pin=-90:{\\texttt{3.108}}] at  (axis description cs:3.108,0){};")


end


function drawB()
local string = string
local lines = {}

for line in io.lines("daten/kahve_kal1.txt") do
  table.insert(lines, line)
end
local summex=0
local summey=0
local xwerte={}
local ywerte={}



for i, line in ipairs(lines) do
  if i>1 then
     local s, e = line:find("%s+", 1)
     local y=tonumber(line:sub(e))
     local x=tonumber(line:sub(1,e))
     table.insert(xwerte,x)
     table.insert(ywerte,y)
   end
end

----m
local n = #lines-1

    tex.sprint("\\addplot[".."no marks".."] coordinates{")
    -- tex.sprint("\\addplot coordinates{")
    for i=1,n do
      tex.sprint("("..xwerte[i]..","..ywerte[i]..")")
    end
    tex.sprint("};")
     tex.sprint("\\node[small dot,pin=-90:{\\texttt{3.108}}] at  (axis description cs:3.108,0){};")


end

function drawC()
local string = string
local lines = {}

for line in io.lines("daten/kahve_kal2.txt") do
  table.insert(lines, line)
end
local summex=0
local summey=0
local xwerte={}
local ywerte={}



for i, line in ipairs(lines) do
  if i>1 then
     local s, e = line:find("%s+", 1)
     local y=tonumber(line:sub(e))
     local x=tonumber(line:sub(1,e))
     table.insert(xwerte,x)
     table.insert(ywerte,y)
   end
end

----m
local n = #lines-1

    tex.sprint("\\addplot[".."no marks".."] coordinates{")
    -- tex.sprint("\\addplot coordinates{")
    for i=1,n do
      tex.sprint("("..xwerte[i]..","..ywerte[i]..")")
    end
    tex.sprint("};")
     tex.sprint("\\node[small dot,pin=-90:{\\texttt{3.108}}] at  (axis description cs:3.108,0){};")


end

function drawD()
local string = string
local lines = {}

for line in io.lines("daten/kahve_kal3.txt") do
  table.insert(lines, line)
end
local summex=0
local summey=0
local xwerte={}
local ywerte={}



for i, line in ipairs(lines) do
  if i>1 then
     local s, e = line:find("%s+", 1)
     local y=tonumber(line:sub(e))
     local x=tonumber(line:sub(1,e))
     table.insert(xwerte,x)
     table.insert(ywerte,y)
   end
end

----m
local n = #lines-1

    tex.sprint("\\addplot[".."no marks".."] coordinates{")
    -- tex.sprint("\\addplot coordinates{")
    for i=1,n do
      tex.sprint("("..xwerte[i]..","..ywerte[i]..")")
    end
    tex.sprint("};")
     tex.sprint("\\node[small dot,pin=-90:{\\texttt{3.108}}] at  (axis description cs:3.108,0){};")


end

function drawE()
local string = string
local lines = {}

for line in io.lines("daten/kahve_kal4.txt") do
  table.insert(lines, line)
end
local summex=0
local summey=0
local xwerte={}
local ywerte={}



for i, line in ipairs(lines) do
  if i>1 then
     local s, e = line:find("%s+", 1)
     local y=tonumber(line:sub(e))
     local x=tonumber(line:sub(1,e))
     table.insert(xwerte,x)
     table.insert(ywerte,y)
   end
end

----m
local n = #lines-1

    tex.sprint("\\addplot[".."no marks".."] coordinates{")
    -- tex.sprint("\\addplot coordinates{")
    for i=1,n do
      tex.sprint("("..xwerte[i]..","..ywerte[i]..")")
    end
    tex.sprint("};")
     tex.sprint("\\node[small dot,pin=-90:{\\texttt{3.108}}] at  (axis description cs:3.108,0){};")


end

function drawF()
local string = string
local lines = {}

for line in io.lines("daten/kahve_kal5.txt") do
  table.insert(lines, line)
end
local summex=0
local summey=0
local xwerte={}
local ywerte={}



for i, line in ipairs(lines) do
  if i>1 then
     local s, e = line:find("%s+", 1)
     local y=tonumber(line:sub(e))
     local x=tonumber(line:sub(1,e))
     table.insert(xwerte,x)
     table.insert(ywerte,y)
   end
end

----m
local n = #lines-1

    tex.sprint("\\addplot[".."no marks".."] coordinates{")
    -- tex.sprint("\\addplot coordinates{")
    for i=1,n do
      tex.sprint("("..xwerte[i]..","..ywerte[i]..")")
    end
    tex.sprint("};")
     tex.sprint("\\node[small dot,pin=-90:{\\texttt{3.108}}] at  (axis description cs:3.108,0){};")


end

function drawG()
local string = string
local lines = {}

for line in io.lines("daten/kahve_kal6.txt") do
  table.insert(lines, line)
end
local summex=0
local summey=0
local xwerte={}
local ywerte={}



for i, line in ipairs(lines) do
  if i>1 then
     local s, e = line:find("%s+", 1)
     local y=tonumber(line:sub(e))
     local x=tonumber(line:sub(1,e))
     table.insert(xwerte,x)
     table.insert(ywerte,y)
   end
end

----m
local n = #lines-1

    tex.sprint("\\addplot[".."no marks".."] coordinates{")
    -- tex.sprint("\\addplot coordinates{")
    for i=1,n do
      tex.sprint("("..xwerte[i]..","..ywerte[i]..")")
    end
    tex.sprint("};")
     tex.sprint("\\node[small dot,pin=-90:{\\texttt{3.108}}] at  (axis description cs:3.108,0){};")


end
\end{luacode*}
\newcommand\addplotb{\directlua{drawB()}}
\newcommand\addplotc{\directlua{drawC()}}
\newcommand\addplotd{\directlua{drawD()}}
\newcommand\addplote{\directlua{drawE()}}
\newcommand\addplotf{\directlua{drawF()}}
\newcommand\addplotg{\directlua{drawG()}}

\pgfplotsset{
  standard/.style={
    axis x line=middle,
    axis y line=middle,
     xlabel= Zeit, % Set the labels
    ylabel= Potential ,
    y unit= \si{\milli\volt} , % Set the respective units
    x unit= \si{\minute},
    legend style={at={(0.5,-0.2)},anchor=north},
    x label style={at={(axis description cs:0.5,-.18)},anchor=south},
        y label style={at={(axis description cs:-0.1,.5)},rotate=90,anchor=south},
    ymin = 29, xmin=0
  }
}
  \tikzset{
    every pin/.style={fill=scarletred1,rectangle,rounded corners=3pt,font=\tiny},
    small dot/.style={fill=chameleon1,circle,scale=0.3}
  }
\begin{document}



%\begin{axis}[standard, xtick={1.27, 4.85 }, xticklabels={ \small{$t_m$= 1.27}, \small{$t_r$ = 4.85}}]





\begin{tikzpicture}
\begin{groupplot}[group style={group size=3 by 2,horizontal sep=15pt,vertical sep=50pt,xlabels at=edge bottom,
ylabels at=edge left,}]

\nextgroupplot[standard,title= \textbf{c=0 \si{\milli\gram\per\liter}},xtick={1.15, 4.95 },xticklabels={$t_m = 1.15$,$t_r = 4.95$ }]
    \addplotb
\nextgroupplot[standard,title= \textbf{c=50 \si{\milli\gram\per\liter}},xtick={1.0, 4.91 },xticklabels={$t_m = 1.0$,$t_r = 4.91$, }]
    \addplotc
 \nextgroupplot[standard,title=\textbf{c=100 \si{\milli\gram\per\liter}},xtick={0.99, 4.88 },xticklabels={$t_m = 0.99$,$t_r = 4.88$, }]
    \addplotd
  \nextgroupplot[standard,title= \textbf{c=150 \si{\milli\gram\per\liter}},xtick={0.98, 4.84 },xticklabels={$t_m = 0.98$,$t_r = 4.84$, }]
    \addplote
        \pgfplotsset{title style={at={(0.5,-0.4)}}}

  \nextgroupplot[standard,title= \textbf{c=200 \si{\milli\gram\per\liter}},xtick={0.96, 4.8 },xticklabels={$t_m = 0.96$,$t_r = 4.8$, }]
    \addplotf
        \pgfplotsset{title style={at={(0.5,-0.4)}}}

  \nextgroupplot[standard,title= \textbf{c=250 \si{\milli\gram\per\liter}},xtick={0.95, 4.79 },xticklabels={$t_m = 0.95$,$t_r = 4.79$, }]
    \addplotg
        \pgfplotsset{title style={at={(0.5,-0.4)}}}

\end{groupplot}
\end{tikzpicture}
%    \pgfplotsset{title style={at={(0.5,-0.4)}}}

\end{document}
