...
\begin{tikzpicture}[scale=2.5,every node/.style={scale=0.6}]
  \begin{axis}[standard,title= Spektrum von Munze, smooth, line width=0.1pt, enlarge x limits={abs=0.2cm},minor x tick num=1, xlabel=$\tilde{\nu}$]
    \addplotpr
    \end{axis}
  \end{tikzpicture}

\begin{tikzpicture}[scale=2.5,every node/.style={scale=0.6}]
  \begin{axis}[standard,title= Spektrum von Munze -Nach Sputter , smooth, line width=0.1pt, enlarge x limits={abs=0.2cm},minor x tick num=1, xlabel=$\tilde{\nu}$]
    \addplotdd
    \end{axis}
  \end{tikzpicture}
