...
function fs.readcsv( file , strip)
  ...
  for i, line in ipairs(lines) do
    ...
    if (i>=strip and not isempty(line) and line:find("%d+") ) then
...
function fs.readanalyse( file , strip ,colx, coly)
  ...
  for i, line in ipairs(lines) do
    ...
    if (i>=strip and not isempty(line) and line:find("%d+") ) then
      ...
      local y=tonumber(columns[coly])
      local x=tonumber(columns[colx])
      table.insert(xvalues,x)
      table.insert(yvalues,y)
    end
